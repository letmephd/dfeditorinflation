...
class DragonPipeline_3d:
    def __init__(self, sd_id='stabilityai/stable-diffusion-2-1', ip_id='models/ip_sd15_64.bin', NUM_DDIM_STEPS=50, precision=torch.float32, ip_scale=0):
        unet = DragonUNet3DConditionModel.from_pretrained_2d(sd_id, subfolder="unet")
        ...
        self.ip_id = ip_id
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained('/data/yinzijin/checkpoints/IP-Adapter', subfolder='models/image_encoder').to('cuda', dtype=precision)
    ...
    def decode_latents(self, latents):
        ...
        return img
    @torch.no_grad()
    def decode_latents_video(self, latents):
        ...
        latents_list = []
        for l in latents_split:
            latents_list.append(self.pipe.vae.decode(l).sample)
        image = torch.cat(latents_list,0)
        # image = self.pipe.vae.decode(latents_split[0]).sample
        # image = torch.cat([self.pipe.vae.decode(l).sample for l in latents_split[0]], dim=0)
        ...
        # from IPython import embed;embed()
        # image = torch.unbind(image, dim=0)
        image = [tensor2img(img)[:,:,::-1] for img in image]
        # if is_video:
    ...
    def ddim_inv(self, latent, prompt, emb_im=None):
        ...
        return ddim_latents
    def ddim_inv_3d(self, latent, prompt, emb_im=None):
        ddim_inv = DDIMInversion(model=self.pipe, NUM_DDIM_STEPS=self.NUM_DDIM_STEPS)
        ddim_latents = ddim_inv.invert(ddim_latents=latent, prompt=prompt, emb_im=emb_im)
        return ddim_latents
    ...
    def load_adapter(self, model_path, scale=1.0):
        ...
        for name in self.pipe.unet.attn_processors.keys():
            # if "_temp" in name:
            #     continue
            if name.endswith("attn_temp.processor"):
                continue
        ...
        new_dict = {}
        for key in self.pipe.unet.attn_processors:
            if "attn_temp" not in key:
                new_dict[key] = self.pipe.unet.attn_processors[key]
        
        ip_layers = torch.nn.ModuleList(new_dict.values())
        ip_layers.load_state_dict(state_dict["ip_adapter"], strict=True)
        # ip_layers.load_state_dict(state_dict["ip_adapter"], strict=True)