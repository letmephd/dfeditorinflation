        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps[-start_time:])):
            ...
            for ri in range(repeat):
                ...
                if energy_scale!=0 and i<30 and (alg=='D' or i%2==0 or i<10):
                    ...
                    if mode == 'move_':
                        guidance = self.guidance_move_(latent=latent, latent_noise_ref=latent_noise_ref[-(i+1)], t=t, text_embeddings=text_embeddings_org, energy_scale=energy_scale, **edit_kwargs)
                ...
                if repeat>1:# 这些步骤都是在中间几个step才会用到的
                    with torch.no_grad():
                        ...
                        beta_prod_t = 1 - alpha_prod_t
                        model_output = self.unet(latent_prev.unsqueeze(2), next_timestep, encoder_hidden_states=text_embeddings,save_kv=False, mode=mode, iter_cur=-2)["sample"].squeeze(2)
